...
```bash
$ dnf install neovim htop terminator google-chrome-stable screenfetch vim git zsh
```

##### 不常用软件

```bash
$ dnf install ca-certificates (ca证书)
$ dnf install wireshark 
$ dnf install libreoffice
$ dnf install smplayer
$ dnf install mirage (图片查看工具)
```
...
```bash
$ wget https://golang.google.cn/dl/go1.19.1.freebsd-amd64.tar.gz
```
